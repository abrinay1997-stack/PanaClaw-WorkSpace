 ...
 * persona.
 *
 * Es de toda la API y no solo del historial: el panel de clientes rechaza con
 ...
 * con el mismo texto, para que la consola no salga vacía.
 */
...
/** El mensaje que se le enseña a una persona, venga el error de donde venga. */
export function mensajeDe(error: unknown, porDefecto: string): string {
  return error instanceof FalloApi ? error.mensaje : porDefecto;
}
